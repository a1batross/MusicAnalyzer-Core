...
then
	...
	#	$CFGOPTIONS -prefix $PREFIX \
	#	-openssl-source $OPENSSL_ROOT \
	#	-script -stl -webkit || exit 1
	$SRC_DIR_QT/configure -v -prefix $PREFIX -opensource -confirm-license \
		$QT_FLAGS -no-largefile -host-arch $HOST_ARCH $PLATFORM -openssl -openssl-source $OPENSSL_ROOT \
		-host-little-endian -little-endian -xplatform android-g++ -nomake demos -nomake examples -nomake tests\
		-arch $ANDROID_ARCHITECTURE || exit 1
fi
...
if [ "$BUILD_QT" = "1" ]; then
	make -f $MAKEFILE -j$JOBS all || exit 1
#	if [ "$OSTYPE" = "msys" ] ; then
		while [ "$?" != "0" ]
		do
